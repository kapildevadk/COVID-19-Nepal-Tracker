...
#To create Database of District wise
df11 = df8[['dis_code','cases']]

df11 = df11.groupby(["dis_code"], as_index=False)[["cases"]].sum()

# merge df1 & df11
df_dist = pd.merge(df11, df1, on='dis_code')

#assign row number
df_dist.insert(0, 'S.N.', range(1, 1 + len(df_dist)))

#RENAME SPECIFIC COLUMNS
df_dist.rename(columns={'dis_en':'District'}, inplace=True)
#COLUMNS
df_dist = df_dist[['dis_code','District','cases','Latitude_dist','Longitude_dist']]

#export database
df_dist.to_csv(r'Nepal_Covid-19_by-District.csv', index=False, header=True)
