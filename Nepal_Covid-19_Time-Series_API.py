...
#merge df
df4 = pd.merge(df3, df2, on='mun_id')

#again, merge df
df5 = pd.merge(df4, df1, on='dis_code')

# #assign row number
# df5.insert(0, 'S.N.', range(1, 1 + len(df5)))

# #Finally assign the columns
# df5 = df5[['S.N.','province','dis_code','dis_en','Latitude_dist','Longitude_dist','mun_id','mun_en','Latitude_mun','Longitude_mun','reportedOn','currentState','gender','age','cases']]
# #export whole spreadsheet type database
# # df5.to_csv(r'Nepal_Covid-19_Timeline_New.csv', index=False, header=True)
...
#assign row number
df8.insert(0, 'S.N.', range(1, 1 + len(df8)))

#export database to csv/xlsx into local drive (r' saves the result in current local directory)
df8.to_csv(r'Nepal_Covid-19_Time-Series.csv', index=False, header=True)
